...
O codec preserva bytes desconhecidos e cauda, valida limites e sobreposições e retorna cópias de bytes. A criação de modelo segue o código canônico. A campanha web agora mantém uma cópia do record 1: sincroniza vidas, diamantes, equipamento, mundos, conclusão, recompensas e saídas secretas. Ao concluir uma fase, registra a quantidade vermelha daquela fase e as coordenadas dos baús abertos. Ao revisitar, os baús consumidos permanecem abertos; esse estado também entra no replay. O laboratório pode importar um payload de record 1 para usar na campanha ou exportar a campanha atual. Isso não inclui o contêiner RMS do emulador nem o sistema de compras e vida máxima acima de quatro pontos do original; bytes importados não interpretados são preservados.

## Replay experimental

JSON com `version: 3`, `target: "1.2.0-s700"`, `engine: "fidelity-18"`, mundo, fase, recursos iniciais (inclusive equipamento e baús já abertos), impressão FNV-1a dos três planos/dimensões e entradas por tick. Edições do mapa feitas por cenas são guardadas junto ao tick correspondente. A impressão detecta diferenças de dados; não é assinatura de segurança. Entradas são reconstruídas executando a mesma simulação desde o começo. Máximo importado: 144000 ticks, ou duas horas a 20 Hz.
